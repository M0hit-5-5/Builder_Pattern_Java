...
public class Demo {
    ...
    private static void buildLaptop() {
        ...
        scan.close();
        if(x == 1){
            director.buildDallruzenLaptop(builder, random.nextInt(100000, 999999));
        ...
        if(x == 2){
            director.buildDallshintelLaptop(builder, random.nextInt(100000, 999999));
            Laptop lap = builder.build();
            builds[buildIndex] = lap;
            buildIndex++;
            return;
        ...
        if(x == 3){
            director.buildPhruzenLaptop(builder, random.nextInt(100000, 999999));
            Laptop lap = builder.build();
            builds[buildIndex] = lap;
            buildIndex++;
            return;
        ...
        if(x == 4){
            director.buildPhshintelLaptop(builder, random.nextInt(100000, 999999));
            Laptop lap = builder.build();
            builds[buildIndex] = lap;
            buildIndex++;
            return;
        ...
        if(x == 5){
            director.buildAsosGamingLaptop(builder, random.nextInt(100000, 999999));
            Laptop lap = builder.build();
            builds[buildIndex] = lap;
            buildIndex++;
            return;
